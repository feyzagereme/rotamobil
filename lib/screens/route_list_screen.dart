                      ...
                      children: [
                        ...
                        if (_isLoadingRoutes)
                          ...
                          ..._backendRoutes.map((route) {
                            ...
                            final stops = routeJson is Map ? (routeJson["stops"] ?? routeJson["addresses"]) : null;
                            final path = routeJson is Map ? routeJson["path"] : null;
                            final stopCount = stops is List
                                ? stops.length
                                : path is List
                                    ? (path.isNotEmpty ? path.length - 1 : 0)
                                    : null;
                            final routeAddresses = stops is List
                                ? stops
                                    .take(3)
                                    .map((e) {
                                      if (e is Map) {
                                        return (e["customerName"] ?? e["address"] ?? e["street"] ?? e["title"] ?? "Durak").toString();
                                      }
                                      return e.toString();
                                    })
                                    .toList()
